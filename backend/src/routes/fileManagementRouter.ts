  ...
  const fileManagementRouter = router({
    createAsset: protectedProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        const { asset, signedUploadUrl } =
          await fileManagementService.createAsset({
            userId: ctx.userId,
    ...
    confirmUpload: protectedProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        if (!asset || asset.userId !== ctx.userId) {
          throw new TRPCError({
            code: "FORBIDDEN",
            message: "Cannot confirm upload for this asset",
          });
        }
        const updated = await fileManagementService.markAssetAsUploaded(
          input.assetId
        );
        if (!updated) {
          throw new TRPCError({
    ...
    getAssetsForUser: protectedProcedure
      ...
      .query(async ({ input, ctx }) => {
        ...
        const assets = await fileManagementService.getUploadedAssetsByUserId(
          input.userId
        );
        ...
        const assetsWithUrls = await Promise.all(
          assets.map(async (asset) => ({
            ...
            downloadUrl: await fileManagementService.generateDownloadSignedUrl(
              asset.s3Key
            ),
          }))
        );
