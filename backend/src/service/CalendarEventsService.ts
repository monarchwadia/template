...
export class CalendarEventsService {
    ...
    private async requireCommunityOwnerOrThrow(communityId: string, userId: string) {
        const community = await this.prisma.community.findUnique({ 
            where: { id: communityId },
            include: { owner: true }
        });
        if (!community) throw new TRPCError({ code: "NOT_FOUND", message: "Community not found" });
        if (community.ownerId !== userId) throw new TRPCError({ code: "FORBIDDEN", message: "Only community owners can manage events" });
        return community;
    }

    private async getCommunityMemberEmails(communityId: string): Promise<string[]> {
    ...
    async viewCalendarEvent(eventId: string, userId?: string) {
        ...
        if (!event.publishedAt && event.community.ownerId !== userId) {
            throw new TRPCError({ code: "FORBIDDEN", message: "Event is not published" });
        }
    ...
    async updateCalendarEvent(eventId: string, data: {
        ...
        if (event.publishedAt) {
            ...
            await this.emailService.sendEventUpdatedEmail(eventId, updatedEvent.title, memberEmails);
        }
    ...
    async cancelCalendarEvent(eventId: string, userId: string) {
        ...
        if (event.cancelledAt) {
            throw new TRPCError({ code: "BAD_REQUEST", message: "Event is already cancelled" });
        }
        ...
        if (event.publishedAt) {
            const memberEmails = await this.getCommunityMemberEmails(event.communityId);
            await this.emailService.sendEventCancelledEmail(eventId, cancelledEvent.title, memberEmails);
        }
    ...
    async publishCalendarEvent(eventId: string, userId: string) {
        ...
        if (event.publishedAt) {
            throw new TRPCError({ code: "BAD_REQUEST", message: "Event is already published" });
        }

        if (event.cancelledAt) {
            throw new TRPCError({ code: "BAD_REQUEST", message: "Cannot publish a cancelled event" });
        }
        ...
        const memberEmails = await this.getCommunityMemberEmails(event.communityId);
        await this.emailService.sendEventPublishedEmail(eventId, publishedEvent.title, memberEmails);
    ...
    async hideCalendarEvent(eventId: string, userId: string) {
        ...
        if (!event.publishedAt) {
            throw new TRPCError({ code: "BAD_REQUEST", message: "Event is not published" });
        }
